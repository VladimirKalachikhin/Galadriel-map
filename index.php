	...
	if($mapList){
		foreach($mapList as $mapName => $mapHumanNames) {
			...
			if($mapName == 'COVER') $isCOVERpresent = true;	// не будем включать в список карт карту покрытия, но отметим, что она есть
				...
				$l = mb_strlen($mapsInfo[$mapName]);
				if($l>$maxStrLength) $maxStrLength = $l;
			};
		};
	};
...
<?php if($privileged and $tileCacheControlURI){	// для пользователя со всеми правами ?>
			<li id="panoControlTab"><a href="#panoControl" role="tab"><img src="img/pano.svg" alt="pano control" width="70%"></a></li>
...
?>
		...
		<div class="leaflet-sidebar-pane" id="panoControl">
			...
			<div id="panoControlPad">
				<table border="0" style="width:9em;margin: 3em auto;" >
				<colgroup>
					<col style="width:10%;">
					<col>
				</colgroup>
				<tr style="height:10%;">
					<td style="text-align:left;"><button style="width:2em;" onClick="sendCameraCommand('UP');"><img src="img/plus.svg" style="vertical-align:middle;height:1em;"></button></td>
					<td style="text-align:center;"><button style="width:5em;" onClick="sendCameraCommand('FORWARD');"><img src="img/triangle-up.svg" style="vertical-align:top;height:1em;"></button></td>
				</tr><tr>
					<td></td>
					<td style="padding:0.5em;text-align:center;">
						<button style="height:3em;margin: 0 0.7em 0 0;" onClick="sendCameraCommand('LEFT');"><img src="img/triangle-left.svg" style="vertical-align:middle;width:1em;"></button>
						<button style="height:3em;margin: 0 0 0 0.7em;" onClick="sendCameraCommand('RIGHT');"><img src="img/triangle-right.svg" style="vertical-align:middle;width:1em;"></button>
					</td>
				</tr><tr style="height:10%;">
					<td style="text-align:left;"><button style="width:2em;" onClick="sendCameraCommand('DOWN');"><img src="img/minus.svg" style="vertical-align:middle;height:1em;"></button></td>
					<td style="text-align:center;"><button style="width:5em;" onClick="sendCameraCommand('BACK');"><img src="img/triangle-down.svg" style="vertical-align:bottom;height:1em;"></button></td>
				</tr>
				</table>
				<div class="parentonoffswitch" style="margin: 0 1em;padding:1em 0;"> <!-- Следовать текущим координатам -->
					<div class="onoffswitch" style="float:right;margin:0 0 0 1em;"> <!--  Переключатель https://proto.io/freebies/onoff/  -->
						<input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" id="panoControlFollowSwitch" onChange="if(panoControlFollowSwitch.checked) sendCameraCommand('followON');
																																else {
																																	sendCameraCommand('followOFF');
																																};" >
						<label class="onoffswitch-label" for="panoControlFollowSwitch">
							<span class="onoffswitch-inner"></span>
							<span class="onoffswitch-switch"></span>
						</label>
					</div>
				...
				<div class="parentonoffswitch" style="margin: 0 1em;padding:1em 0;"> <!-- Kiosk mode -->
					<div class="onoffswitch" style="float:right;margin:0 0 0 1em;"> <!--  Переключатель https://proto.io/freebies/onoff/  -->
						<input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" id="panoControlKioskModeSwitch" onChange="if(panoControlKioskModeSwitch.checked) sendCameraCommand('kioskmodeOFF');
																																else sendCameraCommand('kioskmodeON');" checked>
						<label class="onoffswitch-label" for="panoControlKioskModeSwitch">
							<span class="onoffswitch-inner"></span>
							<span class="onoffswitch-switch"></span>
						</label>
					</div>
...
<?php }; // для пользователя со всеми правами ?>	
		...
		<div class="leaflet-sidebar-pane" id="settings">
			...
			<br>
<?php if($gpsdProxyHost and $gpsdProxyPort){	// Определён сервис координат ?>
			<div class="settingItem"> <?php // Показ целей AIS ?>
...
<?php }; ?>
<?php if($privileged){	// для пользователя со всеми правами ?>
			<div class="settingItem"> <?php // Показ панели управления панорамой ?>
				<div class="onoffswitch" style="float:right;margin: 0 auto;"> <!--  Переключатель https://proto.io/freebies/onoff/  -->
					<input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" id="DisplayPANOswitch" onChange="PANOswitching();">
					<label class="onoffswitch-label" for="DisplayPANOswitch">
						<span class="onoffswitch-inner"></span>
						<span class="onoffswitch-switch"></span>
					</label>
				</div>
			...
			<br>
<?php }; // для пользователя со всеми правами ?>	
			<div class="settingItem"> <!-- Сокрытие элементов управления -->
				<div class="onoffswitch" style="float:right;margin: 0 auto;"> <!--  Переключатель https://proto.io/freebies/onoff/  -->
...
var PANOrole = 'master';
var panoControlMarker;	// маркер указания точки просмотра и направления, будет создан по необходимости.
const hRotateDelta = 15;	// единичный угол поворота панорамы по горизонтали
const vRotateDelta = 3;	// единичный угол поворота панорамы по вертикали
const distanceDelta = 100;	// м, шаг панорамирования
var cameraOptions = {};	// параметры камеры, как они указаны в доке на maplibre
res = storageHandler.restore('DisplayPANOswitch');
if(res != null) DisplayPANOswitch.checked = Boolean(res); 	// показывать управление панорамой
if(DisplayPANOswitch.checked) panoControlTab.style.display='';
else panoControlTab.style.display='none';
...
sidebar.on("content", function(event){ 	// Событие открытия панели с информацией о вкладке. А такого же события закрытия нет.
	...
	case 'routes':	// маршруты
		...
		break;
	case 'panoControl': 	// Управление DEMpanoPanel
		if(CurrnoFollowToCursor === 1)CurrnoFollowToCursor = noFollowToCursor;  // запомним состояние глобального признака следования за курсором, если ещё не запоминали
		if(!panoControlFollowSwitch.checked) noFollowToCursor = true; 	// отключим следование за курсором, если только панорама за ним не следует.
		if(panoControlMarker instanceof L.RotateMarker){	// маркер есть, панель уже открывали
			if(!panoControl.isOpen) {	// однако, панель(и) была закрыта
				//console.log('panoControlMarker в пределах экрана:',isPointInBounds(panoControlMarker.getLatLng(),map.getBounds()));
				if(isPointInBounds(panoControlMarker.getLatLng(),map.getBounds()) !== true){	// маркер панорамы не в пределах экрана
					disablePANOcontrols();	// отключим управление панорамой, тем самым заставив юзера полностью её перерисовать изменением положения маркера
					panoControlMarker.setLatLng(map.getCenter());
				};
			};
			panoControlMarker.addTo(map);
		}
		else{	// первый раз открыли панель
			// Укажем размеры в rem, полагая, что rem == em блока mapid
			// Кроме того, panoPoint центрируется по ширине с помощью text-align, поэтому
			// оно position:relative, а не absolute
			const panoControlIcon = L.divIcon({
				"html" : 
`<div style="text-align:center;margin:0;padding:0;">
	<img src="img/panoPoint.svg" style="width:1.5rem;position:relative;bottom:-8.5rem;margin:0;padding:0;">
	<img src="img/angle37.svg" style="height:7rem;margin:0;padding:0;">
</div>`,
				"className" : "",	// обязательно, иначе будет умолчальный класс, который нарисует белый квадратик
				"iconSize" : null,	// если не указать, то вращение будет вокруг 0,0 Но позиционирование - всё равно относительно 0,0
				"iconAnchor" : null
			});
			// icon в leaflet - это просто объект с текстом html, и до попадания в DOM про него нельзя ничего узнать
			panoControlIcon.options.iconAnchor = [remImPx*7/2,remImPx*8.5+remImPx*1.5-remImPx*1.5/2];	// картинки - квадратные.
			//console.log('panoControlIcon',panoControlIcon);
			
			panoControlMarker = L.rotateMarker(map.getCenter(), {
			//var panoControlMarker = L.marker(map.getCenter(), {
				"icon": panoControlIcon,
				//"bubblingMouseEvents":false,	// чёта не влияет...
				"draggable": true,
			});
			disablePANOcontrols();	// отключить кнопочки управления панорамой, потому что панораме ещё не посылали точку зрения.
			panoControlMarker.addTo(map);
			//console.log('panoControlMarker:',panoControlMarker);
			// Для того, чтобы в объекте можно было сделать CustomEvent, объект должен extends EventTarget
			// и constructor() {super()}. Как это вкорячить в L.RotateMarker - я не понимаю.
			// Поэтому - событие в функции _toggleRotation класса L.RotateMarker, тут же запускается от
			// this._icon, поскольку оно часть DOM и имеет функцию dispatchEvent. А требуемый объект
			// L.RotateMarker передаётся во встроенном объекте detail.
			// Беда этой конструкции в том, что L.RotateMarker не находится по пути всплытия события
			// от this._icon. Я вообще не знаю, где они все находятся, ибо событие удаётся поймать только в
			// document, даже не map.
			//document.addEventListener('markerRotateEnd',function (event){console.log('rotateEnd event:',event.detail.realTarget)});
			// Однако, оказалось, что Агафонкин уже решил все проблемы: имеется fire('markerRotateEnd', {})
			// с произвольным именем события, на которое можно просто подписаться.
			// Конкретно здесь fire - в определении собственно класса L.RotateMarker, поэтому on в экземпляре
			// этого класса имеет в event.target этот же экземпляр.
			// Воистину, Агафонкин был гений до того, как стал украинцем.
			panoControlMarker.on('markerRotateEnd',sendPanoControlMarkerPosition);
			panoControlMarker.on('moveend',sendPanoControlMarkerPosition);
		};
		panoControl.isOpen = true;
		break;
...
sidebar.on("closing", function(event){
	//console.log('sidebar closing',event);	// нельзя узнать, какую именно панель закрыли, но закрываются они все.
	...
	// PANO
	if(panoControlMarker) {	// т.е., если панель панорамы раньше открывали
		//disablePANOcontrols();
		panoControlMarker.remove();	// Убрать маркер управления панорамой
		panoControl.isOpen = false;
	};
...
e.preventDefault();
console.log(e.currentTarget);	// то, на что повешен обработчик, а не то, на что кликнули
const target = e.currentTarget;
if(showMapsToggler.innerHTML == showMapsTogglerTXT[0]) return;	// текущий режим - "избранные карты", в нём не работаем
if(showMapsList.includes(target.id)){	// это избранная карта
	const n = showMapsList.indexOf(target.id);
	showMapsList.splice(n,1);	// вырежем имя из массива
	target.classList.remove("showedMapName");
}
else {
	showMapsList.push(target.id);
	target.classList.add("showedMapName");
}
...
//	console.log('map.on [editable:edited] event.target:',event.target);
//});
...
var subscribe = ['TPV','ATT','AIS','ALARM','WPT','PANO'];	// это полный список возможных подписок. Он уменьшается в зависимости от в spatialWebSocketStart
var WATCHgreeting = {"enable":true,"json":true,"clientName":clientName,"PANOrole":PANOrole,"minPeriod":minWATCHinterval,"subscribe":subscribe.join()};	// это полное приветствие, оно уменьшается в зависимости от
...
var messageQueue = [];	// очередь сообщений
...
if(!DisplayPANOswitch.checked) {
	delete WATCHgreeting.PANOrole;
	subscribe = subscribe.filter(i=>i!='PANO');
};
...
spatialWebSocket.onopen = function(e) {
	...
	checkDataFreshInterval = setInterval(function (){
		...
		if(Date.now()-lastDataUpdate>PosFreshBefore+20000){	// по мере протухания, но не чаще периода проверки
			console.log('The latest data was received too long ago, trying to reconnect for checking.');
			spatialWebSocket.close(1000,'The latest data was received too long ago');
		}
	},30000);	// Проверяем каждые  секунд
	//
...
spatialWebSocket.onmessage = function(event) {
	...
	try{
		...
		console.log('spatialWebSocket: Parsing inbound data error:',error.message);
		return;
	};
	...
	case 'DEVICES':
		...
		break;
	case 'WATCH':
		console.log(`spatialWebSocket.onmessage: Handshaiking with gpsd complit: WATCH recieved. Sending ${messageQueue.length} queued messages.`);
		sendMessage();
		break;
	case 'POLL':
		break;
...
if(DisplayAISswitch.checked) {
	// переподпишемся. Если оно было неуспешно - сработает событие spatialWebSocket.onerror и spatialWebSocket.onclose
	subscribe = subscribe.filter(i=>i!='AIS');
	subscribe.push('AIS');
	sendWATCH();	
}
else {
	// даже если переподписка будет неуспешна, всё равно
...
function PANOswitching(){
if(DisplayPANOswitch.checked) {
	//console.log('[PANOswitching] включили');
	panoControlTab.style.display='';
	WATCHgreeting.PANOrole = PANOrole;
	subscribe = subscribe.filter(i=>i!='PANO');
	subscribe.push('PANO');
	sendWATCH();	// переподпишемся
}
else {
	//console.log('[PANOswitching] выключили');
	panoControlTab.style.display='none';
	delete WATCHgreeting.PANOrole;
	subscribe = subscribe.filter(i=>i!='PANO');
	sendWATCH();	// переподпишемся
};
}; // end function PANOswitching


spatialWebSocketStart(); 	// запускам периодическую функцию получать всё.
<?php }; ?>
...
function realtimePANOupdate(indata){
//console.log('[realtimePANOupdate] PANO data',indata);
if(!panoControlTab || (panoControlTab.style.display=='none')) return;	// вообще нет управления панорамой - пользователь без прав, или выключили
if(Array.isArray(indata.slaves)){	// пришло изменение списка клиентов
	if(indata.slaves.length){	// клиенты есть
		let selected = storageHandler.restore('PANOslaves') || [];	// список сохранённых клиентов
		if(selected.length)	PANOslavesList.innerHTML = indata.slaves.map(item => `<option value="${item}" ${selected.includes(item)?'selected':''}>${item}</option>`).join('\n');	// из indata.slaves создаётся текст html <option..., в котором те <option, имя которых есть в ранее сохранённом списке selected, помечается как selected.
		else {
			PANOslavesList.innerHTML = indata.slaves.map(item => `<option value="${item}">${item}</option>`).join('\n');
			//console.log('[realtimePANOupdate] панорамы есть, но ни одна из них ранее не была выбрана - выбираем первую:',PANOslavesList.options[0].value);
			PANOslavesList.options[0].selected = true;	// выберем первого попавшегося клиента
		};
		if(panoControl.isOpen) panoControlMarker.addTo(map);	// покажем курсор панорамы. Хотя instanceof - лишнее, достаточно проверки наличия.
	}
	else{	// клиентов нет
		PANOslavesList.options.length = 0;	// очистим список
		//storageHandler.save('PANOslaves',[]);	// а надо ли? Запустят показывалку - а у нас с собой?
		disablePANOcontrols();
	};
};
if(!indata.clientFromName) return;	// дальше не будем принимать ничего от клиентов без имени отправителя
// Проверим, отправитель есть ли в списке тех с кем мы общаемся?
let selectedSlave = false;
for (const option of PANOslavesList.selectedOptions) {
	if(option.value == indata.clientFromName){
		selectedSlave = indata.clientFromName;
		break;
	};
};
if(!selectedSlave) return;	// приславший сообщение не в списке тех, с кем мы общаемся
// Если мы не хотим принимать сообщения от панорамы, то надо сказать - отключиться? А юзер
// поймёт? А типа если сначала открыл панель панорамы - значит, точно хочет?
// Если панорама единственная - она всегда подключена.
if(!panoControl.isOpen) return;	// панель управления панорамой закрыта
if(indata.action){	// может быть и список и много команд?
	for(const command in indata.action){
		//console.log('[realtimePANOupdate] PANO command:',command,'panoControl.isOpen=',panoControl.isOpen);
		switch(command){
		case 'POI':
			sidebar.open('measure');	// откроем панель маршрут
			map.setView(indata.action.POI.point);
			break;
		case 'cameraOptions':
			cameraOptions = indata.action.cameraOptions;	// заменим местное значение на пришедшее
			if(indata.action.cameraOptions.cameraCenter) panoControlMarker.setLatLng(indata.action.cameraOptions.cameraCenter);
			if(indata.action.cameraOptions.bearing !== undefined) panoControlMarker.setRotationAngle(indata.action.cameraOptions.bearing);
			if(indata.action.cameraOptions.panoControlFollowSwitch !== undefined) panoControlFollowSwitch.checked = indata.action.cameraOptions.panoControlFollowSwitch;
			if(indata.action.cameraOptions.panoControlKioskModeSwitch !== undefined) panoControlKioskModeSwitch.checked = indata.action.cameraOptions.panoControlKioskModeSwitch;
			enablePANOcontrols();	// включить кнопочки управления панорамой. Типа, панорамы показывают то, на что указывает значёк панорамы.
			break;
		};
	};
};
