...
var realtime = L.realtime(gpsanddataServerURI, {
	...
	updateFeature:  function(feature,olgLayer,newLayer) {
		//console.log(feature);
		olgLayer = cursor;
		return olgLayer.setLatLng(L.latLng(feature.geometry.coordinates[1],feature.geometry.coordinates[0]));
	},
...
setInterval(function(){realtime(gpsanddataServerURI,realtimeUpdate);},1000);

// Realtime периодическое обновление
function realtimeUpdate(gpsdData) {
	// .gps. - это geoJSON id, который отдаёт testGPSD.php
	// Положение неизвестно
	if(gpsdData.features[0] === undefined) { 	// баг в leaflet-realtime : если geometry":null, то вся onUpdate.features.gps неопределена, и прочитать сообщение об ошибке невозможно
	...
	cursor.setLatLng(L.latLng(gpsdData.features[1].geometry.coordinates[1],gpsdData.features[1].geometry.coordinates[0]));
	var positionTime = new Date(gpsdData.features[1].properties.time);
	var now = new Date();
	...
	if((gpsdData.features[1].properties.heading !== null) && Math.round( gpsdData.features[1].properties.velocity ) != 0) {heading = gpsdData.features[1].properties.heading;} // если положение изменилось - возьмём новое направление, иначе - будет старое.
...
<?php } ?>
	...
	var velocity = Math.round((gpsdData.features[1].properties.velocity*60*60/1000)*10)/10; 	// скорость от gpsd - в метрах в секунду
	...
	var velocityCursorLength = gpsdData.features[1].properties.velocity*60*velocityVectorLengthInMn; 	// метров  за  минуты
	velocityCursorLength = Math.round(velocityCursorLength/metresPerPixel);
	//console.log('map.getZoom='+map.getZoom()+'\nmetresPerPixel='+metresPerPixel+'\ngpsdData.features[1].properties.velocity='+gpsdData.features[1].properties.velocity+'\nvelocityCursorLength='+velocityCursorLength);
	...
	var errGNSS = (+gpsdData.features[1].properties.errX+gpsdData.features[1].properties.errY)/2;
