<?php
/**/
function chkPrivileged(){
/* Возвращает true, если адрес, с которого обратились к скрипту, находится в белом списке 
$grantsAddrList из params.php, или не находится в чёрном.
*/
global $grantsAddrList;
...
$ip=trim($ip," \n\r\t\v\x00[]");
if(!$ip) return false;
//echo "[chkPrivileged] ip=$ip;<br>\n";

$res = false;
foreach($grantsAddrList[1] as $range){
	if ((strpos($range, '/') === false) and (strpos($range, '*') === false) and (strpos($range, '-') === false)) {	// указан адрес
		if($ip==$range) $res = true;
	}
	else {	// указан интервал
		if (strpos($ip, ':') === false) {	// адрес ipv4
			$res = ipv4_in_range($ip, $range);
		}
		else {	// адрес ipv6
			$res = ipv6_in_range($ip, $range);
		};
	};
	if($res) break;
};
if($grantsAddrList[1]=='blacklist') $res = !$res;
return $res;
}; // end function chkPrivileged
