    ...
    var t = {
        ...
        gpx: function(doc) {
            var i,
                ...
                gj = fc(),
                feature;
            var    tracks = get(doc, 'trk'); 	// возьмём все треки
            //alert('tracks '+tracks+'\n'+tracks.length);
            if(tracks.length) {
            	var lastPoint = {};
		        for (i = 0; i < tracks.length; i++) {
		            feature = getTrack(tracks[i]);
	            	//alert(lastPoint.coordinates);
		            if (feature) gj.features.push(feature);
		        }
		    }
		    else { 	// в документе не оказалось ни одного трека
	            var segments = get(doc, 'trkseg'); 	// возьмём тогда все сегменты
	            //alert('segments '+segments+'\n'+segments.length);
	            if(segments.length) {
				    for (i = 0; i < segments.length; i++) {
				        feature = getTrackSeg(segments[i]);
				        if (feature) gj.features.push(feature);
				    }
				}
				else { 	// в документе не оказалось ни одного сегмента
		            var points = get(doc, 'trkpt'); 	// возьмём тогда все путевые точки
				    for (i = 0; i < points.length; i++) {
				        feature = getPoint(points[i]);
				        if (feature) gj.features.push(feature);
				    }
				}
		    }
...
//                }
            ...
            function getTrackSeg(node) {
            	/* GeoJSON для набора сегментов.  node - один сегмент */
                var track = [], times = [], line;
                line = getPoints(node, 'trkpt');
