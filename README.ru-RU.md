...
* [Работает в OpenWRT](работ-в-openwrt)
...
## Совместимость
Требуется Linux (OpenWRT, VenusOS) на сервере и более-менее современный браузер на клиентском устройстве.
...
Можно скачать комплект без поддержки векторных тайлов (без шрифтов, значков и библиотек) -- его размер менее 20MB, или полный комплект -- размером более 20MB.
...
Кроме того, GaladrielMap может получать координаты от VenusOS, локально или по сети. Если GaladrielMap не может подключиться к **gpsd**, предпринимается попытка обнаружить VenusOS и получить данные оттуда.
...
*  разумеется, требуется достаточное количество места на карте SD для хранения тайлов.
