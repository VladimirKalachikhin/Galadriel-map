...
## Ограничения
При получении информации от приборов через сеть (от аппаратного концентратора или из среды [Signal K](https://signalk.org/) (оборудование Victron)) в некоторых случаях возможна задержка отображения актуальных значений до нескольких секунд. Это не касается значений координат, скорости и направления, которые всегда правильные.  
Проблема вызвана особенностями функционирования демонов [Signal K](https://signalk.org/) и [gpsd](https://gpsd.io/) и касается сетевых источников данных и данных приборов.  
Использование [gpsdPROXY](https://github.com/VladimirKalachikhin/gpsdPROXY) решает проблему.
...
GaladrielMap получает координаты, скорость, направление от работающего на сервере сервиса [gpsd](https://gpsd.io/).  
 ...
 Также возможно получение координат от сети [Signal K](https://signalk.org/). Если в файле крнфигурации _params.php_ не указать никаких источников геопозиционирования, GaladrielMap будет искать в локальной сети сервисы Signal K и пытаться получить координаты от них.  
 Но всё же лучше указать в _params.php_ адрес сервера Signal K.
...
## Отображение информации AIS 
 ...
 Сбор данных AIS осуществляется от [gpsd](https://gpsd.io/) через [gpsdPROXY](https://github.com/VladimirKalachikhin/gpsdPROXY). Демону **gpsd** может понадобится некоторая настройка для получения данных от приёмопередатчика AIS. Если же имеется только usb приёмник AIS -- никакой настройки обычно не требуется.  
Получение информации AIS по умолчанию отключено. Его можно включить, раскомментировав строку с переменной $aisServerURI в файле _params.php_. Поскольку для получения информации AIS необходим [gpsdPROXY](https://github.com/VladimirKalachikhin/gpsdPROXY) -- установите соответствующие значения переменных $gpsdHost и $gpsdPort в файле _params.php_
