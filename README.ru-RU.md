...
## Возможности:
* [Конфигурируемые пользователем локальные или сетевые карты](#карты)
* * [Карты MAPbox](#карты-mapbox)
* [Отображение положения](#отображение-положения)
* [Запись пути](#текущий-путь)
* [Отображение маршрутов](#отображение-маршрутов), путевых точек и других представляющих интерес объектов
* * [CSV](#csv)
* * [Некоторые источники представляющих интерес объектов](#некоторые-источники-представляющих-интерес-объектов)
* * * [BALTIC BOATHARBORS ANG NATURHARBORS](#baltic-boatharbors-ang-naturharbors)
* * * [NVPEDIA](#nvpedia)
* * * [SaimaaPOI](#saimaapoi)
* [Создание и редактирование маршрута](#создание-и-редактирование-маршрута)
* [Следование по маршруту](#следование-по-маршруту)
* [Ввод и получение координат](#ввод-и-получение-координат)
* [Пустой экран](#пустой-экран)
* [Прогноз погоды](#прогноз-погоды) от  [Thomas Krüger Weather Service](http://weather.openportguide.de/index.php/en/)
* [Отображение информации AIS](#отображение-информации-ais)
* [Обнаружение столкновений](#обнаружение-столкновений)
* [Поддержка netAIS](#поддержка-netais)
* [Режим "Человек за бортом!"](#человек-за-бортом)
* [Разграничение доступа](#разграничение-доступа)
* Отдельная [приборная панель](#приборная-панель), оптимизированная для устройств с экраном на "электронных чернилах" (E-Ink)
* [Панорамный просмотр](#панорамный-просмотр)
* [Управление скачиванием карт](#загрузчик-galadrielcache)
* Интерфейс на русском и других языках
...
## Ограничения
Продукт не работает по протоколу https, только http.
...
[Имеется](https://github.com/VladimirKalachikhin/GaladrielMap-Demo-image/blob/master/README.ru-RU.md) настроенный и готовый к запуску образ виртуальной машины в общепринятом формате, а также загрузочный образ для Raspberry Pi, также настроенный и готовый к работе с первого запуска. Образы содержат GaladrielMap, GaladrielCache, netAIS, gpsd, gpxlogger и всё остальное, что необходимо для функционирования полноценного бортового сервера.
...
## Аварийный набор (или быстрый старт)
...
* [gpsdPROXY](https://github.com/VladimirKalachikhin/gpsdPROXY) в каталоге _gpsdPROXY/_ требуется версия не ниже 1.4
...
* [Leaflet.TextPath](https://github.com/makinacorpus/Leaflet.TextPath/) как _Leaflet.TextPath/leaflet.textpath.js_
...
php chkGPXfiles.php
```
Текущий путь записывается в каталог `tracks/` (в каталог, указанный в переменной $trackDir конфигурационного файла _params.php_).


## Отображение маршрутов
GaladrielMap показывает пути и маршруты а также другие представляющие интерес объекты, записанные в форматах gpx, kml и csv:  
![Display routes and POIs](screenshots/s5.jpeg)<br>
Файлы располагаются на сервере в каталоге `route/` (как это указано в файле конфигурации _params.php_). Откройте панель <img src="img/poi.svg" alt="POI" width="24px">.  
...
Файл с путевыми точками надо положить в каталог `route/` на сервере (в каталог, указанный в переменной $routeDir конфигурационного файла _params.php_). Следует избегать совпадений имён файлов в каталогах `route/` и `tracks/`.

### Некоторые источники представляющих интерес объектов
#### BALTIC BOATHARBORS ANG NATURHARBORS
["В книге (англ. яз.) приводится краткая информация о примерно 1800 местах возможной стоянки яхт (марины и природные стоянки) на побережьях Балтийского моря"](https://parusanarod.ru/bib/books/harbors/index.htm)  
Книга платная, однако имеется бесплатный gpx со всеми описываемыми в книге точками. К сожалению, кроме коодинат, в gpx ничего больше нет.

#### NVPEDIA
[www.free-x.de/nvpedia/](https://www.free-x.de/nvpedia/)  
Иллюстрированный справочник стоянок, причалов и марин немецкого и голландского побережий Балтийского моря, в формате html. На немецком языке, свободный.  
##### Установка справочника в GaladrielMap
* Скопируйте (или создайте ссылку) каталог `nvpedia/` из архива в каталог GaladrielMap (`/map/` по умолчанию).
* Скопируйте (или создайте ссылки) все файлы gpx из архива в каталог `/map/route/`.

Не забудьте назначить права. Откройте панель <img src="img/poi.svg" alt="POI" width="24px">.

#### SaimaaPOI
Список марин и природных стоянках на озере Сайма в Финляндии [SaimaaPOI](https://github.com/VladimirKalachikhin/Saimaa-POI).  
Там же имеется файл csv с точками фотографирования и с ссылками на фотографии.  
...
Панель меню <img src="img/route.svg" alt="Handle route" width="24px"> содержит средства управления следованием оп маршруту. Нарисуйте маршрут, как описано выше, и нажмите кнопку "Следовать". Это пошлёт на [gpsdPROXY](https://github.com/VladimirKalachikhin/gpsdPROXY) команду определить путевые точки и следить за прибытием в них. GaladrielMap будет показывать направление на очередную путевую точку и окружность, определяющую прибытие в неё.  
Можно выбрать следующую или предыдущую путевые точки.
Аналогично можно указать маршрут для следования в имеющемся файле .gpx. Если, конечно, там есть маршрут. Записанный путь в качестве мршрута (пока?) использовать нельзя.  
Можно изменить маршрут, по которому происходит следование, в редакторе, как описано выше. После сохранения на сервере начнётся следование по изменённому маршруту.
...
## Пустой экран
В меню <img src="img/settings1.svg" alt="Settings" width="24px"> можно назначить невидимую зону на экране, касание которой приведёт к исчезновению всех элементов управления. Повторное касание вернёт всё обратно.  
Это может быть удобно при работе с очень маленькими экранами.
...
GaladrielMap поддерживает все возможности сервиса, отображая нестандартные статусы, текстовые сообщения, угрожающие ситуации и ситуацию "Человек за бортом".
...
Кроме того, при получении сообщений AIS Search and Rescue Transmitter (SART) AIS-MOB и AIS-EPIRB также активизируется режим "Человек за бортом!" Однако, в этом случае запись марщрута автоматически не включается.


## Разграничение доступа
Имеется возможность указать адреса и/или подсети, с которых разрешён доступ ко всем аозможностям программы (белый список), или адреса и/или подсети, с которых разрешён только ограниченный доступ (чёрный список).  
Суть ограничений в том, что нет возможности производить действия, приводящие к изменению информации на сервере, за исключением активизации режима "человек за бортом".  
Ограничения указываются в конфигурационном файле [gpsdPROXY](https://github.com/VladimirKalachikhin/gpsdPROXY). Однако, если вы не используете gpsdPROXY, или там не указаны ограничения, вы можете указать их в `params.php`.
...
## Панорамный просмотр
GaladrielMap позволяет управлять средством панорамного просмотра [DEMpanoPanel](https://github.com/VladimirKalachikhin/DEMpanoPanel). Включите эту возможность в меню настроек <img src="img/settings1.svg" alt="Настройки" width="24px">.

Откройте панель управления панорамой <img src="img/pano.svg" alt="Панорама" width="24px">. На карте появится курсор указания точки обзора и направления взгляда. Переместите курсор в нужное место обычным образом. Для поворота курсора щёлкните внутри него, поверните движением мыши или пальца по экрану и щёлкните вторично.  
Панорама отобразит указанную точку зрения. Кнопки со стрелками позволяют менять направление взгляда, не меняя точку зрения. Если эти действия будут совершены непосредственно на экране панорамы, курсор панорамы на экране GaladrielMap изменит своё положение соответствующим образом.

После того, как точка обзора и направление взгляда будут указаны панораме, все маркеры, установленные на карту этим экземпляром GaladrielMap инструментами панели <img src="img/route.svg" alt="Маршрут" width="24px"> "Маршрут" будут также показываться в панорме.  
И наоборот - указание мышью точки на панораме приведёт к открытию панели "Маршрут" и позиционированию перекрестия в указанной точке.  
![Пример изображения маркеров](screenshots/s18_ru.jpeg)<br>
