...
Автор пользуется GaladrielMap на своей яхте Galadriel, загружая приложение с [wi-fi маршрутизатора под управлением OpenWRT](https://github.com/VladimirKalachikhin/MT7620_openwrt_firmware) в качестве сервера. 
...
Имеется настроенный и готовый к запуску [образ виртуальной машины](https://github.com/VladimirKalachikhin/GaladrielMap-Demo-image) в общепринятом формате.
...
При необъодимости, в файлах _askGPSD.php_  и _askAIS.php_  укажите адрес и порт, используемые gpsd. 
...
Для сбора данных AIS используется демон [gpsdAISd](https://github.com/VladimirKalachikhin/gpsdAISd), который запускается в _askAIS.php_. Поэтому в этом файле может понадобится указать имя исполняемого файла интерпретатора php.
